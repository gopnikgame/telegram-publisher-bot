...
PROJECT_DIR="telegram-publisher-bot" # Имя директории проекта
...
update_repo() {
    ...
    # Сохраняем изменения, внесенные в .env
    if [ -f ".env" ]; then
        log "BLUE" "💾 Сохранение изменений, внесенных в .env файл..."
        cp ".env" ".env.tmp" # Создаем временную копию
        backup_restore_env "restore" # Восстанавливаем из бэкапа
        # Объединяем изменения из .env.tmp в восстановленный .env
        # (Этот шаг может быть сложным, в зависимости от формата файла .env)
        # Простой вариант - перезаписать .env.tmp поверх восстановленного .env
        cp ".env.tmp" ".env"
        rm ".env.tmp" # Удаляем временный файл
        log "GREEN" "✅ Изменения в .env сохранены"
    else
...
if [ -d "$PROJECT_DIR" ]; then
    # Если директория существует, переходим в нее и обновляем репозиторий
    log "BLUE" "🔄 Директория проекта существует. Переход в директорию и обновление..."
    cd "$PROJECT_DIR"
    update_repo
else
    # Если директории не существует, клонируем репозиторий
    log "BLUE" "⬇️ Клонирование репозитория в директорию $PROJECT_DIR..."
