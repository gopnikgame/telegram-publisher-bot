...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply_text(
        ...
        f"Тестовый режим: {'включен' если config.TEST_MODE else 'выключен'}"
    )
...
async def toggle_test_mode(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply_text(
        f"✅ Тестовый режим {'включен' если mode == 'on' else 'выключен'}\n"
        f"{'Сообщения будут отправляться в тестовый чат' если mode == 'on' else 'Сообщения будут публиковаться в канал'}"
    )
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        await update.message.reply_text(
            f"✅ Сообщение {'протестировано' если is_test else 'опубликовано'}!"
        )
